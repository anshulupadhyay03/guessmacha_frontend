  ...
  return (
    <section className="w-full" aria-label="GuessMacha home screen">
      <main className="mx-auto w-full max-w-160 flex-1 px-5.5 pt-2 max-[520px]:px-4.5">
        ...
        <button type="button" className="mt-4.5 flex w-full cursor-pointer items-center gap-4 rounded-[18px] border border-white/8 bg-white/3 px-4.5 py-4.5 text-left text-[#f4f1f7] transition hover:-translate-y-px hover:border-[#7fe4dc]/35 max-[520px]:px-3.5" onClick={onCreateGame}>
        ...
        <div className="mt-4.5 pt-2">
          ...
          <input
            ...
            spellCheck={false}
          />
          <button type="button" className="mt-3.5 w-full cursor-pointer rounded-xl bg-linear-to-br from-[#7fe4dc] to-[#6cccff] px-4.5 py-4 text-[1.2rem] font-extrabold text-[#0f1723] transition hover:-translate-y-px hover:shadow-[0_18px_32px_rgba(111,211,225,0.2)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-cyan-200" onClick={handleJoinRoom}>
            Join Room
          </button>
        </div>

        {error && <p className="mt-3.5 text-[0.88rem] leading-5 text-[#f8d1d4]" role="alert">{error}</p>}
      </main>
